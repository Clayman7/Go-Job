...
function Paginated() {
  ...
  return (
       <div className="flex justify-center bg-white rounded-lg font-[Poppins]">
         <button onClick={back} className="h-12 border-2 border-r-0 border-blue-800
         ...
         {
            pages.map((pg, i) => (
               <button key={i} onClick={() => setCur(pg.page)} className={`h-12 border-2 border-r-0 border-blue-700
               w-12 ${cur === pg.page && 'bg-blue-800 text-white'}`}>{pg.page}</button>
            ))
         }
         <button onClick={Next} className="h-12 border-2  border-blue-800
